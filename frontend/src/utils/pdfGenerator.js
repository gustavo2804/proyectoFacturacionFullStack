...
export const generateCotizacionPDF = (cotizacion, cliente, detalles, productos, itbisRate = 0.18) => {
  ...
  // TABLA DE PRODUCTOS
  const tableHeaders = ['Concepto', 'Cantidad', 'Precio Unit.', 'Total'];
  const columnWidths = [85, 25, 35, 30];
  const tableStartX = 20;
  const tableWidth = columnWidths.reduce((a, b) => a + b, 0);
  let tableX = tableStartX;
  ...
  if (detalles.length > 0) {
    addText('Esta cotización tiene una validez de 30 días desde la fecha de emisión.', tableStartX, currentY, {
      fontSize: 9,
      color: COLORS.text
    });
    
    currentY += 6;
    addText('Los precios están sujetos a cambios sin previo aviso.', tableStartX, currentY, {
      fontSize: 9,
      color: COLORS.text
    });
  ...
  // Texto del footer
  addText(`Cotización generada el: ${new Date().toLocaleDateString('es-DO')}`, pageWidth / 2, footerY, {
    fontSize: 8,
    align: 'center',
    color: COLORS.text
  });
  ...
  // Guardar el PDF
  const fileName = `Cotizacion_${cotizacion.numero_cotizacion}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
